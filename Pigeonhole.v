...
Proof.
  intro eq_dec;
  revert l1;
  induction l2 as [|y l2 IH]; intros l1 H H0.
  - simpl in H0; nia.
  - destruct l1 as [|x l1].
    ...
    + assert (H2: In y (x::l1)). apply H; left; auto.
      ...
      destruct IH with (l1a ++ l1b).
      * intros z H3; apply in_or_app;
        rewrite H2 in H; specialize (H z);
      ...
      * assert (H3: length l1 = length (l1a ++ l1b)). {
          rewrite app_length.
          assert (H3: length ([x] ++ l1) = length (l1a ++ [y] ++ l1b)).
      ...
      * destruct H3 as [l2b [l2c [z H3]]]; subst;
        exists (y::x0), l2b, l2c, z; auto.
Qed.