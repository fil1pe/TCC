...
Lemma pumping_buffer1 q : forall w,
  n1_lower_bounded -> ixtransition w = q -> q <> (sink) ->
  (length w >= length (Q))%nat ->
  exists w', ixtransition w' = q /\ (length w' < length w)%nat /\
  count_buffer w' <= count_buffer w.
Proof.
  intros w H H0 H1 H2;
  assert (H10: exists m, Z.of_nat m >= - (n1 - n0 - count_buffer w - 1)). {
    assert (H3: 0 >= (n1 - n0 - count_buffer w - 1)).
      unfold n1_lower_bounded, is_generated in H; rewrite <- H0 in H1; apply H in H1; omega.
    assert (H4: 0 <= -(n1 - n0 - count_buffer w - 1)).
      omega.
  ...
  - exists (w1++w3); repeat split.
    ...
    + rewrite H2.
      ...
      omega.
  - assert (contra: n0 + count_buffer (w1 ++ word_pow w2 (S m) ++ w3) < (n1)). {
...
Lemma min_list_correct1 l : forall q,
  l <> nil ->
...
Definition min_count_event := count_buffer (min_in_list (E)).

Lemma min_count_event_correct : forall e,
  In e (E) -> count_event e >= count_buffer (min_in_list (E)).
Proof.
  intros e H.
...
Definition min_gen_words q := min_list (all_gen_words_le q (length (Q) - 1)).

Lemma min_gen_words_correct q :
  n1_lower_bounded -> is_tangible q ->
...
Proof.
  ...
  - intros w H1;
    ...
    + apply min_list_correct, all_gen_words_le_correct; auto.
    + assert (H3: (length w >= length (Q))%nat). omega.
      eapply pumping_buffer with (q:=q) in H3.
      2-4: auto.
      destruct H3 as [w' [H3 [H4 H5]]].
      assert (count_buffer w' >= count_buffer (min_list (all_gen_words_le q (length (Q) - 1)))).
        apply min_list_correct, all_gen_words_le_correct; auto; omega.
      omega.
  - apply min_list_correct1.
...
Definition tangible_min_word q w := is_tangible q -> min_word q w.

Lemma exists_min_word_function : n1_lower_bounded ->
    exists f, forall q, is_tangible q -> min_word q (f q).
Proof.
  intro H.
  exists (min_gen_words);
  intro q;
  intro H0;
  apply min_gen_words_correct; auto.
Qed.

Lemma min_word_q0 : forall w,
  n1_lower_bounded ->
  min_word (q0) w ->
  count_buffer w = 0.
Proof.
...
Theorem iff_exists_function__upper_bounded :
  ( exists f, f (q0) <= (n0) /\ forall q, In q (Q) -> is_tangible q -> f q >= n1 /\
    forall e, In e (E) -> f (delta q e) <= f q + count_event e )
  <-> n1_lower_bounded.
Proof.
  split.

  - unfold n1_lower_bounded; intros [ f [H H0] ] w H1.
    cut (n0 + count_buffer w >= f (ixtransition w)).
    cut (f (ixtransition w) >= n1).
    omega.
    apply H0.
  ...
  - intro H.
    pose H as H0; apply exists_min_word_function in H0; destruct H0 as [f H0];
      unfold tangible_min_word in H0.
    exists (fun q => if A_decidable q (sink) then (n1)+(min_count_event) else (n0) + count_buffer (f q));
    split.
    ...
    + intros q H1 H2; pose H2 as H3; apply H0 in H3; destruct (A_decidable q (sink)).
      ...
      * intros e H5; destruct (A_decidable (delta q e) (sink)) as [H7|H7].
        assert (count_event e >= min_count_event).
          apply min_count_event_correct; auto.
        omega.
        ...
        assert (count_buffer (f (delta q e)) <= count_buffer (f q) + count_event e).
          eapply min_word_transition. 2: apply H7. reflexivity. apply H0; auto. apply H0; auto.
        omega.
Qed.
