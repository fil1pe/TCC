...
Haja vista que não é possível definir funções parciais no assistente de provas Coq, algumas mudanças na definição de AFD supracitada são necessárias a fim de representar AFDs nessa ferramenta. A começar, é impreterível alterar a função de transição de um AFD $G$ para torná-la total. Seja $\delta' : Q \cup \{ \otimes \} \times E \rightarrow Q \cup \{ \otimes \}$ a seguinte função total: $$\delta'(q, e) = \begin{cases}
\delta(q, e) & se$ $\delta(q, e) \in Q \\
\otimes & se$ $\delta(q, e) \not\in Q
\end{cases}$$ em que $\otimes$ é algum estado novo, não pertencente a $Q$.
...
Como a função $\delta'$ é total, tem-se que $$L(G') = E^\star$$ em termos do que se estabeleceu como linguagem gerada por AFD anteriormente. Pode-se, não obstante, modificar a definição de linguagem gerada de forma que $G$ e $G'$ sejam equivalentes em se tratando de linguagem. Seja a linguagem $$L'(G') = \{ w \in E^\star \mid \hat{\delta}(q_0, w) \in Q \wedge \hat{\delta}(q_0, w) \neq \otimes \}$$ então $$L'(G') = L'(G) = L(G)$$
