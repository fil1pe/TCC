...
\citeonline{cassandras} citam, como exemplos de SEDs, os sistemas de filas, computacionais, de comunicação, de manufatura e de tráfego. A exemplo disso, protocolos de comunicação em redes de computadores são frequentemente modelados por SEDs, como indica a Figura \ref{fig:protocolo_rede}. O presente trabalho enfoca os sistemas relacionados à automação industrial, para os quais a modelagem de filas é conveniente. A espera de entidades por um recurso em uma fábrica é exemplo que justifica isso, porque a abstração por meio de filas ou \textit{buffers} é muito útil nesse caso. O Capítulo \ref{cap:filas} explanará sobre essa e outras situações em que se pode abstrair a noção de filas.

\figura{Protocolo para um canal de redes de computadores com erros de bits: lado remetente (acima) e destinatário (abaixo)}{
...
Um autômato finito não determinístico (\acs{AFND}) é um modelo diferenciado com a capacidade de estar em mais de um estado ao mesmo tempo, segundo \citeonline{hopcroft}. Essa característica configura a predição dessas máquinas abstratas, permitindo estimar os possíveis estados em que um SED se encontra. Para um AFND $G = \langle Q, E, \delta, q_0, Q_m \rangle$, a função de transição é do tipo $\delta : Q \times Q \to 2^Q$, retornando um conjunto de estados.
...
\citeonline{hopcroft} demonstram a conversão de qualquer AFND $G$ para um AFD $G'$ usando a construção de subconjuntos, de forma que $L(G) = L(G')$ e $L_m(G) = L_m(G')$. Assim, convertendo os modelos, é possível usar os teoremas do Capítulo \ref{cap:propriedades} para verificar propriedades de sistemas modelados por AFND.
...
\figuradoautor{AFD cujos tipos dos estados e eventos são \texttt{states} e \texttt{events} respectivamente}{
	\begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto] 
	\node[state] (q0) {\texttt{q0}}; 
	\node[state] at (2,0) (q1) {\texttt{q1}}; 
	\node[state,accepting] at (4,0) (q2) {\texttt{q2}};
	\draw
	(q0) edge node{\texttt{a}} (q1)
	(q1) edge[bend left, above] node{\texttt{b}} (q2)
	(q2) edge[bend left, below] node{\texttt{a}} (q1);
	\end{tikzpicture}
}{fig:afd_coq_cristiano}

O diagrama da Figura \ref{fig:afd_coq_cristiano} não ilustra o estado de ralo, \texttt{sink\_state}, evidenciando apenas os estados próprios, \texttt{proper\_state}.