...
Proof.
  ...
  - exists (w1++w3); repeat split.
    ...
    + rewrite H2, count_buffer_distr, count_buffer_distr, count_buffer_distr;
      omega.
  - assert (contra: init_items + count_buffer (w1 ++ word_pow w2 (S m) ++ w3) > max_items). {
...
(* If the queueing system is upper bounded, for every generated sequence of events w with length greater or equal
...
(* max_word q w is True if w counts the minimum items in q (maximum word) *)
...
Definition max_gen_words q := max_list (all_gen_words_le q (length states - 1)).

(* If the queueing system is upper bounded, every tangible state q is upper bounded. max_gen_words q must
...
Proof.
  ...
  - intros w H1;
    ...
    + apply max_list_correct, all_gen_words_le_generated; auto.
    + assert (H3: (length w >= length states)%nat). omega.
      eapply pumping_buffer with (q:=q) in H3.
      2-4: auto.
      destruct H3 as [w' [H3 [H4 H5]]].
      assert (count_buffer w' <= count_buffer (max_list (all_gen_words_le q (length states - 1)))).
        apply max_list_correct, all_gen_words_le_generated; auto; omega.
      omega.
Qed.

(* The initial state's maximum word does not add items to the buffer if the queueing system is bounded. *)
Lemma max_word_state0 : forall w,
  upper_bounded ->
  max_word state0 w ->
  count_buffer w = 0.
Proof.
...
Proof.
  ...
  - intro H.
    exists (fun q => if A_eq_dec q sink then (max_items)+(max_count_event) else (init_items) + count_buffer (max_gen_words q)).
    split.
    + destruct (A_eq_dec state0 sink) as [H2|H2].
      ...
      apply max_word_state0 with (w:=max_gen_words state0) in H.
      omega.
    ...
    + intros q H0 H4; pose H4 as H1; destruct H1 as [H1 [w H2]].
      ...
      assert (H5: generates (max_gen_words q)). {
          apply (max_gen_words_correct q) in H. 2: auto.
      ...
      * intros e H3;
        ...
        assert (init_items + count_buffer (max_gen_words q) <= max_items). auto.
        omega.
        assert (count_buffer (max_gen_words (transition q e)) >= count_buffer (max_gen_words q) + count_event e). {
          apply max_word_transition with (q:=q)(qe:=transition q e); auto.
          1,2: apply max_gen_words_correct; auto.
          split. auto.
