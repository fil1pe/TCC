...
(* the queueing system is upper bounded if all of its generated sequences of events respect the number
  of maximum items in the buffer *)
Definition upper_bounded := forall w, generates w -> init_items + count_buffer w <= max_items.

(*
  We can construct a function f that gives the maximum number of items in the buffer in a given state following
 this constraints:
  -> f applied to the initial state gives a number greater or equal to the number of initial items
  -> for every tangible state q, f applied to every transition starting from q through e gives a number greater or
    equal to f applied to q plus count_event e
  --
  We shall now prove that this constraints are correct.
*)

(* The number of items counted from a given generated sequence of events must be less or equal to f applied
...
(*
  ...
  2 - count_buffer w2 > 0 and the queueing system is not upper bounded
*)
Lemma pumping_buffer1 q : forall w,
  upper_bounded -> ixtransition w = q -> q <> (sink) ->
  (length w >= length states)%nat ->
  exists w', ixtransition w' = q /\ (length w' < length w)%nat /\
  count_buffer w' >= count_buffer w.
Proof.
  intros w H H0 H1 H2;
  assert (H10: exists m, Z.of_nat m >= max_items - init_items - count_buffer w + 1). {
    assert (H3: 0 <= (max_items - init_items - count_buffer w + 1)).
      unfold upper_bounded, generates in H; rewrite <- H0 in H1; apply H in H1; omega.
  ...
  - exists (w1++w3); repeat split.
    ...
    + rewrite H2, count_buffer_distr, count_buffer_distr, count_buffer_distr;
      omega.
  - assert (contra: init_items + count_buffer (w1 ++ w2^(S m) ++ w3) > max_items). {
...
(* If the queueing system is upper bounded, for every generated sequence of events w of length greater or equal
  to the number of states, exists a sequence of events that transition into ixtransition w and counts a number of
  items greater or equal to count_buffer w. *)
...
(* max_string q w is True if w counts the minimum items in q (maximum string) *)
Definition max_string q w :=
  ixtransition w = q /\ forall w', ixtransition w' = q -> count_buffer w' <= count_buffer w.

(* a function that takes w1 and w2 and returns the sequence that counts most items *)
Definition max w1 w2 :=
  if Z_ge_dec (count_buffer w1) (count_buffer w2) then w1 else w2.

(* a function that returns the sequence of (possible) events of a given list that counts most items *)
...
(* Every sequence of (possible) events in l must count a number of items less or equal to count_buffer (max_list l). *)
...
(* If all the sequences of events in l transitions into q, max_list l does to. *)
Lemma max_list_state l : forall q,
  l <> nil ->
...
(* a function that returns the [(possible) event] of a given list that adds most items *)
...
(* Every (possible) event in l must add a number of items less or equal to count_buffer (max_in_list l). *)
...
(* count_buffer applied to the event that adds most items to the buffer *)
Definition max_count_event := count_buffer (max_in_list events).

(* Every event must add a number of items less or equal to max_count_event. *)
Lemma max_count_event_correct : forall e,
  In e events -> count_event e <= max_count_event.
Proof.
  intros e H.
...
(* a function that returns the generated sequence of events w of length less than the number of states
  such that w transitions into q and adds most items *)
Definition max_gen_strings q := max_list (all_gen_strings_le q (length states - 1)).

(* If the queueing system is upper bounded, every tangible state q is upper bounded. max_gen_strings q must
  count the maximum items in q. *)
Lemma max_gen_strings_correct q :
  upper_bounded -> is_tangible q ->
  max_string q (max_gen_strings q).
Proof.
  intros H H0.
  unfold max_string.
  split; unfold max_gen_strings.
  - apply max_list_state.
  ...
  - intros w H1;
    ...
    + apply max_list_correct, all_gen_strings_le_generated; auto.
    + assert (H3: (length w >= length states)%nat). omega.
      eapply pumping_buffer with (q:=q) in H3.
      2-4: auto.
      destruct H3 as [w' [H3 [H4 H5]]].
      assert (count_buffer w' <= count_buffer (max_list (all_gen_strings_le q (length states - 1)))).
        apply max_list_correct, all_gen_strings_le_generated; auto; omega.
      omega.
Qed.

(* The initial state's maximum string does not add items to the buffer if the queueing system is bounded. *)
Lemma max_string_state0 : forall w,
  upper_bounded ->
  max_string state0 w ->
  count_buffer w = 0.
Proof.
...
Lemma max_string_transition : forall q e qe w we,
  transition q e = qe -> qe <> sink ->
  max_string q w -> max_string qe we ->
...
(* Now we prove that f exists and, for every tangible state q, f q must be less than max_items iff
  the queueing system is correctly upper bounded. *)
Theorem iff_exists_function__upper_bounded :
  ( exists f, f state0 >= (init_items) /\ forall q, In q states -> is_tangible q -> f q <= max_items /\
    forall e, In e events -> f (transition q e) >= f q + count_event e )
  <-> upper_bounded.
Proof.
  split.

  - unfold upper_bounded; intros [ f [H H0] ] w H1.
    cut (init_items + count_buffer w <= f (ixtransition w)).
    cut (f (ixtransition w) <= max_items).
    omega.
    apply H0.
  ...
  - intro H.
    exists (fun q => if A_eq_dec q sink then (max_items)+(max_count_event) else (init_items) + count_buffer (max_gen_strings q)).
    split.
    + destruct (A_eq_dec state0 sink) as [H2|H2].
      ...
      apply max_string_state0 with (w:=max_gen_strings state0) in H.
      omega.
      apply max_gen_strings_correct; auto.
    + intros q H0 H4; pose H4 as H1; destruct H1 as [H1 [w H2]].
      destruct (A_eq_dec q sink) as [H3|_]. contradiction.
      assert (H5: generates (max_gen_strings q)). {
          apply (max_gen_strings_correct q) in H. 2: auto.
      ...
      * intros e H3;
        ...
        apply max_count_event_correct in H3.
        assert (init_items + count_buffer (max_gen_strings q) <= max_items). auto.
        omega.
        assert (count_buffer (max_gen_strings (transition q e)) >= count_buffer (max_gen_strings q) + count_event e). {
          apply max_string_transition with (q:=q)(qe:=transition q e); auto.
          1,2: apply max_gen_strings_correct; auto.
          split. auto.
